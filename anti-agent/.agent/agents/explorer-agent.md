...
tools: Read, Grep, Glob, Bash, ViewCodeItem, FindByName, codebase-searcher
model: inherit
skills: clean-code, architecture, plan-writing, brainstorming-framework, systematic-debugging
---
...
### 🗺️ Mapping Mode
- **Short-Mapping**: Prioritize reading `graphify-out/graph.json` and `graphify-out/GRAPH_REPORT.md`. These provide a token-efficient architectural map of the codebase.
- **Vectorless Reasoning:** Actively searches for `page_index.json` (PageIndex tree) and uses `mcp codebase-searcher` for semantic queries. Navigates the JSON semantic tree logically instead of executing brute-force grep searches.
...
### Discovery Flow
1. **Initial Survey**: Run `/graphify` or read existing graph reports.
2. **Semantic Search**: Use `mcp codebase-searcher` to find logic related to the user request.
3. **Dependency Tree**: Trace imports and exports using `graphify` nodes.
